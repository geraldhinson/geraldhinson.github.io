...
————
Chapter #2 - A microservice origin story

Everyone likes a good story, right? Especially a true one.

So, let start this discussion reexamining microservices with a story - But, WARNING, this is a true story that may cause you to reconsider how you build apps, as it did for me. 

Several years ago, a group of MSFT veterans was commissioned to solve the mess that internal commerce had grown into at MSFT - all 16 systems. As we all know, tactical systems built quickly to solve short-term problems (then hacked to death to deal with different scenarios and edge cases) can get out of hand. This case was no different. The internal commerce systems had become a costly, confusing mess for both employees and customers alike. It needed fixing and fast.

Everyone agreed on the problem - less so the big hopes for a solution.

But, surprisingly to many, a few short years later, the commissioned team’s goal to reduce all 16 into a single, universal store was completed. It was a huge success - and it was completed in far less time than most believed possible.
...
— Many present shared stories of recovering from crashes, failovers, and the like, wherein least-common-denominator semantics had limited scale and/or required painful data reconciliation/recovery times.

3) Databases were easily best-of-class in terms of most of the attributes from 2) above. Another aspect significantly better for databases was the attention/expertise devoted to provisioning & monitoring them by OPS (and dev) teams. (Interestingly, it was also noted that having more than one type of DB could still result in LCD/mixed semantics pain.)
...
5) In the case of microservice (or adjacent) based approaches, many services that began as simple "nouns (resources)" would quickly become dominated by query logic that changed significantly how the underlying DB performed, scaled, etc. Worse, not only was the creation/maintenance of this code costly, it was consistently a favorite attack vector for hackers exploiting sql injection type attacks..

Overall, the team shared a great dissatisfaction with the state of app complexity, reliability, and scale. 

From this forest-level evaluation sprang the questions, 
- “Does it really have to be this bad?”
- “Can this be simplified?" 

Also noteworthy was the question, “Are there fundamental primitives or patterns that could simplify building apps?"

After several weeks of this, they emerged, convened the broader team, and shared the following (paraphrased): 
...
In REST-speak, these are resources. Common examples are Orders, Payments, Subscriptions, Entitlements, Customers, ServiceRequests, Questionnaires, Answers, Chats, ScheduledJobs, etc.
...
This creates an in-order ledger (history) of each instantiated resource. The journal is useful for:
...
While Get (Resource By Id) and Get (Resources By Owner) are supported, what you will not find in noun services is support for all sorts of random GETs filtering across resources with (in SQL terms) a variety of WHERE clauses.

As mentioned earlier, the architecture team noticed in many cases that these sorts of GETs often grew to dominate the writing and maintenance of many noun services. Another takeaway was that these queries rarely, if ever, needed to be done as part of the natural lifecycle of a resource (ie. when it was created, modified, or deleted).

Further, trying to support such filtered GETs typically prompted “shredding” of an (e.g. JSON) object into multiple tables with their own indexes on write operations, and also necessitated complicated joins to reconstitute an object for a GET.

I'll also note here (again, for emphasis) that app-specific database calls in edge-callable services are where most SQL injection attacks occur.

So, the filtered GETs with costly serialization and joins spanning multiple tables and indexes were removed.

By removing filtered GETs from noun services:
...
- the usage of ORMs (and/or custom database logic or both ) in noun services was eliminated because it was no longer needed

Instead, noun services used a standard resource DB library. JSON objects were stored in a column as text (nope, not even those cool JSON columns were used because they were slower). The average number of writes (tables+indexes) was reduced - usually significantly. The serialization of objects in memory for SELECT/UPDATE/INSERT (which, btw can be more costly than serializing to/from JSON) was eliminated. Support for optimistic locking, an important but often unimplemented feature, was added for the benefit of all noun services in the resource library as well.

In essence, they morphed the definition of a microservice from “single-focused piece of code that has its own database” to “single-focused, journaled piece of code that is wholly contained in two standardized tables”.

Noun services became more useful, faster, consumed fewer resources, and were simpler to scale due to predictable DB usage. Writing & maintaining them became far simpler, too.

(word limit - con't in Part #4)

Add image showing two tables: R & J
...
So, if all of the filtered GET logic has been unceremoniously ripped out of noun services, where does it go? Obviously, it is needed.

This is the realm of the query service.

The query service is a write-once and reuse service (or library) that can be instantiated as many times as are useful. The realization reached by the journaled microservice architecture team was this: 

— There is rarely a need to write custom database logic in an app. This logic is a prime candidate to be centralized, written once, and driven via configured mappings.

The query service accepts HTTP GET calls in standard URL form and replies with JSON results.

(NOTE: I will refer to JSON throughout this writeup, given its current popularity. But the patterns discussed here work equally well with other representations and serialization strategies.)
...
The corresponding HTTP response? A JSON array containing the employees.

Internal to the query service are:

- a ‘canned’ set of queries / stored-procs that are mapped to the API-exposed (ie. URL-form) queries
...
- monitoring of query execution times with appropriate logging/notifications for excessive execution times
...
- Query services can (sometimes) be instantiated to read from DB replicas that have less load and which never interfere with OLTP traffic - like taking orders from customers.

- Can be leveraged in cloud or on-prem, etc.

- Provides a focal point (read: gateway) for DB experts to review all queries for proper indexing/performance. NOTE: Keeping the defined query mappings in GitHub-managed files (versus putting them into a DB table) is safer and simplifies review and history/audit.
...
Step 1 - Create the query and/or stored proc that will be used to retrieve the data. Check plan generation details for it against a production-populated table (ie. a table with ‘real’ rows of content vs one mostly empty. Otherwise, the plan generated may surprise you!) Test run it to ensure it is safe/performant. This is where to leverage your DB experts, if not for creation, then definitely for review.

Step 2 - Define a new query service exposed query mapping (URL-to-SQL) with any associated parameters that must be passed to execute it. This is typically done in a resource file that is part of a given query service’s project. I’ve also seen this broken into separate files - one for public queries and another for secured queries.
...
(For reference, here is a matching URL call that matches the mapping just defined:
...
In my experience, building this service usually takes about 2 weeks, and then it is reused with minor changes only for the next several years. In my teams, it has also become a powerful tutorial for up-and-coming junior engineers seeking to understand the nuances of writing database code correctly.
...
When was the last time you had the opportunity to invest 2 weeks (heck, make it a month!) of one good developer’s time for such an enormous return on investment?

To quote that little goblin who hangs out in Booty Bay in World of Warcraft: Time is money, friend!

(word limit - con't in Part #5)
(need image(s) for query service)
(need image for the “time is money goblin”)
